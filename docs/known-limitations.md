...
- The 0.2 preview provides Windows, Linux, and macOS lifecycle tools. Hosted Linux has real-Docker task gates, but Codex 0.151.0 currently hits the upstream `bwrap` loopback/network-namespace failure there. That hosted environment records `agent_execution_supported: false`; Docker evidence does not replace an Agent sandbox pass.
- With Codex 0.151.0, native Windows may fail to apply the deny-read ACL or may apply it without hiding Factory answers from the Agent. The no-model preflight records either exact condition as `known_unsupported` and stops; it never falls back to unrestricted execution. Windows remains usable for installation, the UI, Blueprint/Case management, deterministic validation, and protocol import/export.
- macOS has hosted build/plugin/service lifecycle and no-model read-isolation evidence only; no physical Mac, Docker Desktop, or authenticated Codex task was used. Treat it as a lifecycle preview, not a proven end-to-end task-execution platform.
...
- On 2026-08-31 the current native-Windows Codex path reproduced `windows sandbox: helper_unknown_error: apply deny-read ACLs`, while hosted Windows accepted the profile but still read the product database. Both preflights made zero model calls. These are unsupported platform capabilities, not passing Agent-execution evidence or fresh-Windows acceptance.
