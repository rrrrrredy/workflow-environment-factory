...
from synthetic_server import blueprint_payload, create_repository, make_correct, mark_synthetic_attempt
...
def main() -> None:
    ...
    try:
        ...
        assert services.scorer.score(code_wrong.run_id, allow_synthetic_fixture=True)["task_result"]["status"] == "fail"
        ...
        assert (
            services.scorer.score(code_correct.run_id, allow_synthetic_fixture=True)["task_result"]["status"]
            == "pass"
        )
        ...
        assert (
            services.scorer.score(issue_wrong.run_id, allow_synthetic_fixture=True)["task_result"]["status"]
            == "fail"
        )
        ...
        assert (
            services.scorer.score(issue_correct.run_id, allow_synthetic_fixture=True)["task_result"]["status"]
            == "pass"
        )
        output = {
